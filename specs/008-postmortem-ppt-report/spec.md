...
- ~~**FR-002**: El informe DEBE mostrar los mismos 8 KPIs globales que muestra el dashboard de postmortem para esa release...~~ — **rediseñado**: el informe muestra solo 3 KPIs (Total Incidencias, % Resueltas PaP, % Resueltas Mesa), calculados a partir de `releases-data.js` (no del JSON de postmortem). % Resueltas PaP y % Resueltas Mesa se muestran en verde si alcanzan el objetivo (`KPI_TARGET_PCT`, 75%) o en rojo si quedan por debajo; el objetivo se muestra explícitamente en la tarjeta ("Objetivo: 75%").
- ~~**FR-003**: El informe DEBE incluir también las 4 gráficas propias del dashboard de postmortem para esa release~~ — **eliminado tras la implementación**: el informe final no incluye ninguna gráfica ni KPI derivado del JSON de postmortem; todo su contenido (KPIs y gráficas) sale de `releases-data.js`.
- ~~**FR-004**: Los valores de KPI del informe DEBEN calcularse con la misma lógica que ya usa el dashboard de postmortem...~~ — **superado por el rediseño**: los KPIs del informe ya no se comparan con el dashboard de postmortem, sino con el dashboard de KPIs de Release (mismos `pct_pap`/`pct_first_week`/`total_incidencias` que la tabla y las gráficas de `dashboards/release-kpis/`).
- **FR-005**: El informe DEBE incluir la gráfica comparativa "Incidencias por release" del dashboard de KPIs de Release, mostrando el conjunto general de releases (no filtrada a una sola release) — se muestra junto a las 3 tarjetas de KPI en la diapositiva de Métricas Globales.
- **FR-006**: El informe DEBE incluir las gráficas comparativas de % de resolución PaP y % de resolución en 1ª semana del dashboard de KPIs de Release, mostrando el conjunto general de releases — ambas juntas en la diapositiva final del informe.
