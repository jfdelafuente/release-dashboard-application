...
from report_generator.chart_utils import BASE_LAYOUT, COLOR_ORANGE, COLOR_INK, COLOR_ORANGE_LIGHT

KPI_TARGET_PCT = 75  # Igual que KPI_TARGET_PCT en app.js — ver discrepancia documentada en spec.md
...
def build_incidencias_por_release_chart(releases):
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[f"{r['pct_pap']}%" for r in releases], textposition="top center", yaxis="y2",
    ))
    fig.add_trace(go.Scatter(
        ...
        text=[f"{r['pct_first_week']}%" for r in releases], textposition="bottom center", yaxis="y2",
    ))
    fig.update_layout(
        **BASE_LAYOUT, legend=_LEGEND, margin=_MARGIN, barmode="stack", hovermode="x unified",
        xaxis=dict(title="Release", tickangle=-45, showgrid=True, gridcolor="#E2DDD5"),
        yaxis=dict(title="Incidencias", showgrid=True, gridcolor="#E2DDD5"),
        yaxis2=dict(title="% Resuelto", overlaying="y", side="right", range=[0, 105], showgrid=False),
    )
    return fig


def _build_kpi_chart(releases, entrada_field, resueltas_field, pct_field, line_color, window_label):
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[f"{v}%" for v in pct], textposition="top center", yaxis="y2",
    ))
    fig.add_trace(go.Scatter(
        x=names, y=[KPI_TARGET_PCT] * len(names), name=f"Objetivo {KPI_TARGET_PCT}%", mode="lines",
        line=dict(color="#1D8754", width=1.5, dash="dash"), yaxis="y2",
    ))
    fig.update_layout(
        **BASE_LAYOUT, legend=_LEGEND, margin=_MARGIN, barmode="stack", hovermode="x unified",
        xaxis=dict(title="Release", tickangle=-45, showgrid=True, gridcolor="#E2DDD5"),
        yaxis=dict(title="Incidencias", showgrid=True, gridcolor="#E2DDD5"),
        yaxis2=dict(title=f"% {window_label}", overlaying="y", side="right", range=[0, 105], showgrid=False),
    )
    return fig


def build_kpi_pap_chart(releases):
    return _build_kpi_chart(releases, "pap_entrada", "pap_resueltas", "pct_pap", COLOR_ORANGE, "PaP")


def build_kpi_post_chart(releases):
    return _build_kpi_chart(releases, "post_entrada", "post_resueltas", "pct_first_week", COLOR_INK, "1ª semana")
